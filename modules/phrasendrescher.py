...
def setup_quoter(ts3bot, db):
    ...
    curs = conn.cursor()
    curs.execute(
        'CREATE TABLE IF NOT EXISTS Quotes (id integer primary key, quote text, submitter text, time text, shown integer)')
...
def add_quote(sender, msg):
    if len(msg) <= len('!quote '):
        ...
        c.execute('INSERT INTO Quotes (quote, submitter, time, shown) VALUES (?, ?, strftime("%s", "now"), ?)',
                  (quote, submitter, 0))
        conn.commit()
        conn.close()
